...
public class RedisUtils {
     ...
     * @return
     */
    public boolean delKeys(Set<String> keys) {
     ...
     * @return
     */
    public boolean clearKeys(String key) {
        Set<String> keys = keys(key);
        if (CollUtil.isNotEmpty(keys)) {
            boolean b = delKeys(keys);
            return b;
        }
        return false;
    }
    ...
    public <T> Map<String, T> hget(String redisKey, Class<T> cls) {
        Map<Object, Object> hmget = hmget(redisKey);
        if (hmget != null) {
            Map<String, T> map = new HashMap<>(hmget.size());
            Set<Object> objects = hmget.keySet();
            for (Object key : objects) {
                Object o = hmget.get(key);
                map.put((String) key, (T) o);
            }
            return map;
        }
        return null;
    ...
    public <T> T hget(String redisKey, String item, Class<T> cls) {
        return (T) hget(redisKey, item);
    }
     ...
     * @return true 成功 false 失败
     */
    public boolean hmsetO(String key, Map<Object, Object> map) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            return true;
        } catch (Exception e) {
     ...
     * @return
     */
    public long clearSet(String key) {

        try {
            Set<Object> valSet = sGet(key);
            if (CollUtil.isNotEmpty(valSet)) {
                Long count = redisTemplate.opsForSet().remove(key, valSet);
                return count;
            } else {
                return 0;
            }
